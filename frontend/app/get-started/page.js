    ...
    <div className="min-h-screen bg-gradient-to-b from-cream to-warm py-12 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          <span className="chip bg-amber-200 text-amber-700 mb-4">{t("getstarted.chip", "CREATING AN ACCOUNT")}</span>
          <h1 className="font-display font-extrabold text-3xl sm:text-4xl md:text-5xl leading-tight tracking-tight mb-3 break-words">
            {t("getstarted.title_1", "What brings you to")}<br className="sm:hidden" /> <span className="text-amber-500">ValueWeave</span>?
          </h1>
          <p className="text-muted max-w-lg mx-auto">
            {t("getstarted.subtitle", "An account is for working with people — collaborators, teams and posting your own ideas. Pick what fits best.")}
          </p>
          ...
          <div className="mt-6 rounded-2xl border border-teal-200 bg-teal-50/60 px-5 py-4 max-w-lg mx-auto">
            <p className="font-display font-bold text-sm text-ink mb-1">
              {t("getstarted.explore_free_title", "Just want to look around?")}
            </p>
            <p className="text-sm text-muted mb-3">
              {t("getstarted.explore_free_desc", "Every skill, scheme, business idea and district is free to read — no account, no sign-in.")}
            </p>
            <Link
              href="/knowledge"
            ...
            >
              {t("getstarted.explore_free_cta", "Explore without an account →")}
            </Link>
          </div>

          <p className="mt-4 text-sm text-stone-500">
            {t("getstarted.already_member", "Already a member?")}{" "}
            <Link href="/signin" data-testid="getstarted-signin-link" className="inline-flex items-center min-h-[44px] text-amber-600 font-semibold hover:underline">
              {t("getstarted.signin_link", "Sign in →")}
            </Link>
          </p>
        </div>
          ...
          {INTENTS.map((it) => {
            ...
            return (
              ...
              >
                ...
                <div className="font-display font-bold text-sm mb-1">{t(`intent.${it.id}_title`, it.title)}</div>
                <div className="text-xs text-muted leading-relaxed">{t(`intent.${it.id}_desc`, it.desc)}</div>
                {isSel && <div className="mt-3 text-xs font-bold text-amber-600">{t("getstarted.selected", "✓ Selected")}</div>}
              </button>
            );
          })}
        </div>
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          >
            {loading ? t("getstarted.redirecting", "Redirecting…") : t("getstarted.continue_google", "Continue with Google →")}
          </button>
          <p className="text-xs text-stone-400">{t("getstarted.no_password", "We'll sign you in with Google. No password needed.")}</p>
        </div>
      </div>
    </div>
  );

# Integration Checklist — Package003_Healthcare v1.0.0
...
2. [ ] Follow `imports/import_sequence.json` for load order (datasets are independent in RC1/RC2, so order does not affect referential integrity).
...
8. [ ] If wiring this package's `state`/`district` fields to Package001_Geography, note there are no FK columns yet in RC1/RC2 — any join must be done by name-matching with manual review.
...
12. [ ] Do NOT promote this package to Stable/merge to `main` without explicit review approval — RC2 is a review candidate, not a final release.

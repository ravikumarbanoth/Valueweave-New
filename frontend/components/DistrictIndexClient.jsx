...
export default function DistrictIndexClient({ telangana, ap }) {
  const { t } = useLanguage();
    ...
    <main>
      <section className="relative overflow-hidden bg-ink px-4 sm:px-6 py-16">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          <span className="chip bg-teal-500/20 text-teal-300 border border-teal-500/30 mb-4">{t("districts.intel_badge", "DISTRICT INTELLIGENCE")}</span>
          <h1 data-testid="district-index-title" className="font-display font-extrabold tracking-tight text-3xl sm:text-4xl md:text-5xl text-white leading-tight mb-4">
            {t("districts.index_title", "Know Your District. Build with Confidence.")}
          </h1>
          <p className="text-white/60 text-base sm:text-lg max-w-2xl mx-auto">
            {t("districts.index_subtitle", "Deep-dive profiles on population, industries, schemes, opportunities, and skills demand — built for Bharat's builders.")}
          </p>
        </div>
      </section>
        ...
        {districts.map((district) => (
          ...
          >
            ...
            <div className="mt-3 flex items-center gap-1 text-amber-700 text-sm font-display font-bold">
              {t("districts.view_profile", "View Profile")}
